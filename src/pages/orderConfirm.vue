      ...
      <div class="container">
        <div class="order-box">
          <div class="item-address">
            ...
            <div class="addr-list clearfix">
              <div class="addr-info" :class="{'checked':index==checkIndex}" @click="checkIndex=index" v-for="(item,index) in list" :key="index">
                ...
                <div class="action">
                  <a href="javascript:;" class="fr" @click="delAddress(item)">
                    <svg class="icon icon-del">
                      <use xlink:href='#icon-del'></use>
                    </svg>
                  </a>
                  <a href="javascript:;" class="fl" @click="editAddressModal(item)">
          ...
          <div class="detail">
            ...
            <div class="item-toatl">
              <span class="item-name">应付金额: </span>
...
export default {
    ...
    data(){
      return{
        ...
        showEditModal:false,//是否新增或编辑弹框
        checkIndex:0//当前收货地址索引
      }
    },
    mounted(){
      this.getAddressList();
    ...
    methods:{
      ...
      getCartList(){
        this.axios.get('/carts').then((res)=>{
          let list=res.cartProductVoList;//获取购物车中所有商品数量
          this.cartTotalPrice=res.cartTotalPrice;//商品总金额
          this.cartList=list.filter(item=>item.productSelected);//过滤属性.过滤商品信息,返回结算商品
          this.cartList.map((item)=>{
            this.count+= item.quantity;//循环商品.拿到结算总件数
          })
        })
      },
      //新增地址弹框 开启
      ...
      //删除地址 弹框开启
      delAddress(item){
        this.checkedItem=item;//把当前商品显示为被选中状态
        this.userAction=2;//此处等于2.调用submitAddress方法,就是删除功能
        this.showDelModal=true;//显示删除弹框
      },

      //地址的删除,编辑.新增.一个函数.根据传值的不同.执行不同操作
      submitAddress(){
      ...
      closeModal(){
        ...
        this.userAction="";
        this.showDelModal=false;//当关闭弹框状态时.选中状态取消.模态框取消.状态恢复点击前的样式
      },
      //订单提交
      orderSubmit(){
